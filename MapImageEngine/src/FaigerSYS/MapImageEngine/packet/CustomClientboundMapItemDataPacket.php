...
class CustomClientboundMapItemDataPacket extends DataPacket {
	...
	public static function prepareColors(array $colors, int $width, int $height) {
		...
		for ($y = 0; $y < $height; $y++) {
			for ($x = 0; $x < $width; $x++) {
				$buffer->putUnsignedVarInt(Binary::flipIntEndianness($colors[$y][$x]));
			}
		}
	...
	public static function checkCompatiblity() : bool {
		...
		$custom->colors = str_repeat(Binary::writeUnsignedVarInt(Binary::flipIntEndianness($color->toRGBA())), 128 * 128);
